...
try {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS registered_admin (
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $check_sql = "SELECT username, email FROM registered_admin WHERE username = ? OR email = ?";
    ...
    // Insert new user
    $sql = "INSERT INTO registered_admin (full_name, username, email, phone, password) 
            VALUES (?, ?, ?, ?, ?)";
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            $stmt->close();
            $conn->close();
            header("Location: ../../iclude/dashboard.php");
            ob_end_clean();
            exit();
        } else {
